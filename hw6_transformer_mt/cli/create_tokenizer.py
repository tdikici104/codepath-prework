...
def main():
    ...
    # 2. Build a target tokenizer,
    target_tokenizer.pre_tokenizer = Whitespace()
    
    # 3. Train it on args.target_lang.Convert to transformers.PreTrainedTokenizerFast and save to save_dir/target_tokenizer
    trainer = BpeTrainer(special_tokens=["[UNK]", "[CLS]", "[SEP]", "[PAD]", "[MASK]", "[BOS]", "[EOS]"])
    
    # 4. Convert to transformers.PreTrainedTokenizerFast 
    target_tokenizer.train(files=[args.target_lang], trainer=trainer)
    
    # 5. Convert the tokenizer to a format compatible with Hugging Face's transformers library.
    target_tokenizer_wrapper = PreTrainedTokenizerFast(tokenizer_object=target_tokenizer,
                                                       bos_token="[BOS]", eos_token="[EOS]", unk_token="[UNK]",
                                                       sep_token="[SEP]", cls_token="[CLS]", pad_token="[PAD]",
                                                       mask_token="[MASK]")
    
    # 6.  save to save_dir/target_tokenizer
    target_tokenizer_wrapper.save_pretrained(os.path.join(args.save_dir, f"{args.target_lang}_tokenizer"))
