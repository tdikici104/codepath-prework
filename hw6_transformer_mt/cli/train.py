...
def main():
   ...
   from transformers import PreTrainedTokenizerFast
    ...
    # Load source and target tokenizers from the variables above
    src_tokenizer = PreTrainedTokenizerFast.from_pretrained(src_tokenizer_path)
    
    # using transformers.PreTrainedTokenizerFast.from_pretrained
    tgt_tokenizer = PreTrainedTokenizerFast.from_pretrained(tgt_tokenizer_path)
    ...
    # Provide all of the TransformerLM initialization arguments from args.
    model = EncoderDecoderModel.from_encoder_decoder_pretrained(args.encoder_model_name, args.decoder_model_name)
    ...
    # (readability matters)
    # YOUR CODE STARTS HERE
   # Assuming args.output_dir, args.source_lang, and args.target_lang are defined
    src_tokenizer_path = os.path.join(args.output_dir, f"{args.source_lang}_tokenizer")
    tgt_tokenizer_path = os.path.join(args.output_dir, f"{args.target_lang}_tokenizer")
    ...
    model = EncoderDecoderModel.from_encoder_decoder_pretrained(args.model_name_or_path, args.model_name_or_path)
model.to(args.device)


    collate_fn = partial(
        collation_function_for_seq2seq_wrapped,
        source_pad_token_id=source_tokenizer.pad_token_id,
        target_pad_token_id=target_tokenizer.pad_token_id,
    )
